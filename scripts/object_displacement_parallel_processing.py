'''object_displacement_parallel_processing.py : Reads images from a folder containing frames from a video
...
'''Function name : draw_ROI_rectangles2
   Description : Draws roi using opencv. Takes the ROIs(the large areas of interest around the templates) and
   draws a rectange that can narrow down the area of interest to where the image is possibily located
   Input parameters :
   image : Image on which drawing has to be done
   rects : Coordinates of the rectangular ROI as [(x1, y1), (x2, y2)]
   color : BGR colors as a tuple (B,G,R)
   Output parameters :
   image : Image with rectange drawn on it'''
def draw_ROI_rectangles2(image, rects, color):
    x_distance=(rects[1][0]-rects[0][0])/4
    y_distance=(rects[1][1]-rects[0][1])/4
    x1=rects[0][0]+x_distance
    y1=rects[0][1]+y_distance
    x2=rects[1][0]-x_distance
    y2=rects[1][1]-y_distance
...
'''Function name : final_centroid_decision
   Description : centre shifting of the centroid coordinates 
   Input parameters :
   index : Index of the template, cooresponding to quadrant number
   leftx : x coordinate of the top left point of the template 
   lefty : y coordinate of the top left point of the template
   centre_x : Centre x coordinate of the template, used for dividing the ROI into 4 quadrants
   centre_y : Centre y coordinate of the template used for dividing the ROI into 4 quadrants
   text : String giving information about movement
   list_centroids : List having centroids
   Output parameters :
   reconverted_x : x coordinate of the centroid where motion is confirmed
   reconverted_y : y coordinate of the centroid where motion is confirmed'''
...
'''Function name : main_processor_with_tagging
   Description : Standalone main function to run the program along with tagging and images stored in a folder_path
   Input parameters : 
   folder_path : Path of folder where images have been stored'''
def main_processor_with_tagging(folder_path):
